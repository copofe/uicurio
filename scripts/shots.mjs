...
const items = readdirSync("data/items").filter((f) => f.endsWith(".json")).map((f) => readJson(`data/items/${f}`));
const targets = only ? items.filter((i) => i.slug === only) : items.filter((i) => !existsSync(`assets/shots/${i.slug}.webp`));
...
async function capture(item) {
  ...
  try {
    ...
    await sharp(buf).resize({ width: 1600 }).webp({ quality: 78 }).toFile(`assets/shots/${item.slug}.webp`);
    ok.push(item.slug);
