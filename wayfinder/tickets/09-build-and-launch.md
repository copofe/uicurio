...
- **剩余（主人的三步，约 10 分钟）**：① Cloudflare Registrar 买 uicurio.com（.dev 建议一并）② GitHub 建仓 push + Cloudflare Pages 连接（构建命令 `npm run build`，输出 `dist/`）③ CF 控制台开 Web Analytics，把 beacon 换入 Base.astro 注释处；域名到手后在 Pages 自定义域名里接上。
